...
js_model = json.load(open('MxnetModel/model-symbol.json', 'r'))
mx_model, arg_params, aux_params = mx.model.load_checkpoint('MxnetModel/model', 191)
...
def show_difference(node):
    ...
    image_nd = mx.nd.array(image_npy * 1.0,ctx=mx.gpu(gpu_id))
    eval_iter = mx.io.NDArrayIter(data=image_nd,batch_size=1)
    mod = mx.mod.Module(symbol=sym,context=mx.gpu(gpu_id))
    ...
    print(diff)
    print('Mean abs error: ',np.mean(diff))
